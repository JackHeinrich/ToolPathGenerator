...
def UploadDocument(path: str):
    texts = break_down_pdf(path)
    docsearch = Pinecone.from_texts([t.page_content for t in texts], embeddings, index_name=index_name)

#setting docsearch regardless of adding new doc or not.
docsearch = Pinecone.from_existing_index(index_name=index_name, embedding=embeddings)
...
DocumentSearchingTool = Tool("DocumentSearchingTool",DocumentSearcher.get_similar_docs,"Use this tool always. Useful for pulling information from documents in our database. Input should be in the form of a question asking for the information you want.")

tools.append(DocumentSearchingTool)

agent_chain = initialize_agent(tools, llm, agent=AgentType.CONVERSATIONAL_REACT_DESCRIPTION, verbose=True, memory=memory)

def ClearIndex(index_name_to_clear: str):
    index_to_clear = pinecone.Index(index_name_to_clear)
